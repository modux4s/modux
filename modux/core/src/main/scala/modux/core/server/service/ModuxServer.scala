...
import modux.model.{RestInstance, RestProxy, ServiceDescriptor}
...
case class ModuxServer(appName: String, host: String, port: Int, appClassloader: ClassLoader, devMode: Boolean = true){
  ...
  Http().bindAndHandle(capture.routes, host, port).onComplete {
  ...
  private def captureCall(context: Context, localConfig: Config): Capture = {
    ...
    val ctxWired: Wired = wiredInModule(context)
    ...
    localConfig
      ...
      .map(c => ctxWired.wireClassInstance(c).asInstanceOf[ModuleX])
      .foreach { x =>
    ...
    //    Capture(cache(lfuCache, keyerFunction)(Directives.concat(routes: _*)), modules, specs)

    if (devMode) {
//      import ch.megard.akka.http.cors.scaladsl.CorsDirectives._
      Capture(CORSHandler.corsHandler {
